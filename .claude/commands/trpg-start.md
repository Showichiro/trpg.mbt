...
- player handoff では `trpg prompt player <name> --brief` の JSON に含まれる `private_handoff_text` を主情報源にしてください。`handoff_text` は互換 alias です。`text` や `--human --brief` は GM inspection 用です。初回呼び出し時は、その PC に subagent 用 play_style がランダム割当され、session 中は永続化されます。加えて、subagent には毎ターン hidden な quality / immersion の runtime tuning が与えられます。必要なら末尾に `## GM補足` セクションを足し、箇条書きで選択肢候補を書いて構いません。
...
- `interaction_mode=mixed_interest` の scenario では、subagent の `非公開意図` を卓上に流さず、`公開行動` だけを戻してください。
