...
`prompt player --human --brief` を初めて呼ぶと、その PC が subagent 運用対象とみなされ、session 内に play_style がランダム割当される。同じ PC はその session 中ずっと同じ style を使う。style は提案傾向にだけ効き、数値補正ではない。style 由来で突飛な案が出ても、卓破壊でない限りは即却下より先に「既存 scene tag に丸める」「prep に落とす」「難度を上げて受ける」の順で拾えるかを検討する。`prompt player --human --brief` の内容をそのまま渡す。必要なら GM が現在の見え方や選択肢を補足してよいが、prompt の事実を上書きしない。シーン遷移直後、援護ターン、分岐判断では末尾に `## GM補足` セクションを足し、その下に箇条書きで候補 `(a)(b)(c)` を書く。full prompt が必要なときだけ `--brief` を外す。subagent 返答末尾の `agentId: ...` 行は無視してよい。
