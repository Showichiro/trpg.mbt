...
- 準備・援護は原則 `trpg roll <name> --prep ...` を使う。`--prep` は target 未指定時に scene target を 2 下げ、scene target が無いときは 9 を使う。
...
- 準備判定や援護判定の成功は、原則 `trpg roll ... --grant-to 対象 --grant tech:+1@ritual,seal` で `uses=1` の一時 status を自動付与して表現する。手動なら本命ロール側へ積み、本命ロールの `stat` に合わせた `--modifier <stat>:+N` で積む。兼用したいなら別 status を積む。
