...
- scenario が未指定なら、まず `trpg scenario list` で導入済み候補を確認する。空なら `trpg scenario bundled` を見て、候補を選んで `trpg scenario install <scenario_id>` を実行してから使う。導入済み候補が 1 件だけならそれを使い、複数あるならユーザにどれを使うか確認する。
...
7. 参加 PC として確定した名前だけを登録する。
   - sample PC は `trpg scenario install <scenario_id>` によって `${TRPG_HOME:-$HOME/.trpg}/scenarios/` へ展開される
   - 例:
     - `trpg pc add alice --template alice`
     - `trpg pc add orion --template orion`
   - 判定ルール:
...
- シナリオ定義を事前確認したいときは `trpg scenario show <scenario_id>` を使う。
- 外部 scenario や CLI 単体配布を使うときは `trpg scenario bundled` / `trpg scenario install` / `trpg scenario list` / `trpg scenario import` / `trpg scenario show` を優先し、repo 直下の `scenarios/*.json` を前提にしない。
