...
- 判定は必ず `trpg roll 2d6+修正値 --target N --as 名前 --context "状況"` を実行する。
- trait を修正値に採用したときは、その理由を `--context` に含める。セッション内で同系統の判断基準を揃える。
...
- 構造化できる状態は `--modifier ability:+N --uses N --duration text --on-trigger consume|persist` も使う。
- アイテム獲得・喪失は `trpg item give/drop 名前 アイテム名 [--note "理由"]` を使う。status で代用しない。
- シーン転換時は `trpg session scene <id> [--note "変化"]` を使う。`trpg scene set` は互換 alias としてのみ扱う。
...
プレイヤー subagent から返るのは「行動宣言」と、必要なら「判定タイプの提案」まで。判定、数値処理、状態更新、シーン更新は GM が CLI で実行する。
