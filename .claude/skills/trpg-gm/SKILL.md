...
- `margin=-1..-2` は軽減して扱ってよい。`special=fumble` は一段重く扱う。
...
`prompt player --human --brief` の内容をそのまま渡す。必要なら GM が 1〜2 文だけ現在の見え方を補足してよいが、prompt の事実を上書きしない。full prompt が必要なときだけ `--brief` を外す。
