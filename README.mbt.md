...
player subagent への handoff には `trpg prompt player <name> --brief` の JSON に含まれる `handoff_text` を使います。`text` や `trpg prompt player <name> --human --brief` は GM inspection 用で、hidden runtime tuning は含みません。`handoff_text` には現在シーン、パーティ状態、直近履歴、active status、play style に加えて、そのターン限りの quality / immersion の hidden tuning が入ります。運用モード指定があればそれに従ってください。シーン遷移直後、援護ターン、分岐判断では末尾に次のような `## GM補足` セクションを足すと安定します。
...
subagent の返答をそのまま機械要約せず、GM はまず卓上で起きたこととして再表現します。意図、しぐさ、短い台詞、場の変化を 1〜3 段落で描写し、その後に必要なら `判定: ...` を 1 行だけ添える形が安定します。候補提示も同様で、scene id や tag、grant/status 名、`判定回避` のような内部語は見せず、世界内の選択肢として書く方が体験が自然です。
